...
- Для SCSS:
  ...
  - не группировать весь адаптив одного брейкпоинта в один общий `@media`-блок в конце файла; медиаправила размещать рядом с конкретными селекторами по месту их использования.
...
- Всегда использовать `rem`-единицы для размеров, отступов и типографики вместо `px` (кроме случаев, где `px` технически обязателен, например для `border` в `1px`).
- Не использовать `height` и `min-height` для задания размеров блоков, если можно достичь результата через `padding`, `gap`, `aspect-ratio`, контентную высоту или другие нефиксированные способы.
- Не использовать `position: absolute`, если задача решается штатной раскладкой (`flex`, `grid`, потоком документа); абсолютное позиционирование применять только при реальной необходимости.
- Если адаптив запрошен, делать его только для мобильного брейкпоинта.
- Для `grid`-раскладок предпочтительно использовать единицы `fr` вместо фиксированных значений, если это не противоречит макету/задаче.
...
- Для оберток изображений предпочтительно использовать `aspect-ratio`, а не фиксированную `height` (если это не противоречит макету/задаче).
